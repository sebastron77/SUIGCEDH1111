...
$area_operador = isset($_GET['a']) ? $_GET['a'] : '0';
?>
<?php header('Content-type: text/html; charset=utf-8');
if (isset($_POST['add_informe_areas'])) {
    ...
    if (empty($errors)) {
        ...
        $move2 =  move_uploaded_file($temp2, $carpeta . "/" . $name2);
            ...
            $query .= "folio, no_informe, fecha_informe, fecha_entrega, informe_adjunto, area_creacion,id_user_creador";
			if($name){
				$query .= ",oficio_entrega ";				
			}
            $query .= ") VALUES (";
            $query .= " '{$folio}','{$no_informe}','{$fecha_informe}','{$fecha_entrega}','{$name2}', '{$area_operador}','{$id_user}'";
			if($name){
				$query .= ",'{$name}' ";				
			}
            $query .= ")";
        ...
        if ($db->query($query) && $db->query($query2)) {
            ...
            redirect('informes_areas.php?a='.$area_operador, false);
        } else {
            //failed
            $session->msg('d', ' No se pudo agregar el informe de actividades.');
            redirect('add_informe_areas.php?a='.$area_operador, false);
        }
    } else {
...
<div class="row">
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            <form method="post" action="add_informe_areas.php?a=<?php echo $area_operador?>" enctype="multipart/form-data">
                <div class="row">
                    <div class="col-md-2">
                ...
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="file" accept="doc, .docx, .pdf" class="form-control" name="informe_adjunto" id="informe_adjunto" required>
                        </div>
                    </div>
                </div>
                <div class="form-group clearfix">
                    <a href="informes_areas.php?a=<?php echo $area_operador?>" class="btn btn-md btn-success" data-toggle="tooltip" title="Regresar">
                        Regresar
                    </a>
