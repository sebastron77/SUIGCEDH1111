...
?>
<?php
$e_detalle = find_by_id('recomendaciones_generales', (int) $_GET['id'], 'id_recom_general');
if (!$e_detalle) {
...
$acuerdos_recomendaciones = find_acuerdo_rec((int) $_GET['id'],'g');
...
if (isset($_POST['add_acuerdo_rec_gral'])) {

    if (empty($errors)) {
        $id_rec = remove_junk($db->escape($_POST['id_recom_general']));
		...
		$carpeta = 'uploads/recomendacionesGenerales/acuerdos/'. $id_rec;
        ...
        $query = "INSERT INTO acuerdos_recomendaciones (id_recom_general,numero_recomendacion, nombre_acuerdo, fecha_acuerdo, acuerdo_adjunto,
        ...
        if ($db->query($query)) {
            ...
            insertAccion($user['id_user'], '"' . $user['username'] . '" agregó acuerdo a recomendación general, Folio: ' . $numero_recomendacion . '.', 1);
            redirect('add_acuerdo_rec_gral.php?id=' . (int) $e_detalle['id_recom_general'], false);
...
<div class="row">
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            <form method="post" action="add_acuerdo_rec_gral.php?id=<?php echo (int) $e_detalle['id_recom_general']?>" enctype="multipart/form-data">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <?php if (!$ac['acuerdo_adjunto'] == "") { ?>
                                            <a target="_blank" href="uploads/recomendaciones/acuerdos/<?php echo $ac['id_recom_general'] . '/' . $ac['acuerdo_adjunto']; ?>" title="Ver Acuerdo">
                                        ...
                                        <?php if (!$ac['acuerdo_adjunto_publico'] == "") { ?>
                                            &nbsp;&nbsp;&nbsp;
                                            <a target="_blank" href="uploads/recomendaciones/acuerdos/<?php echo $ac['id_recom_general'] . '/' . $ac['acuerdo_adjunto_publico']; ?>" title="Ver Versión Publica del Acuerdo">
                                    ...
                                    <td>
                                        <a href="delete_acuerdo_rec.php?id=<?php echo (int) $ac['id_acuerdo_rec']; ?>" class="btn btn-delete btn-md" title="Eliminar" data-toggle="tooltip" onclick="return confirm('¿Seguro(a) que deseas eliminar el Acuerdo?');">
                                            <span class="glyphicon glyphicon-trash"></span><?php insertAccion($user['id_user'], '"' . $user['username'] . '" eliminó el acuerdo de la recomendación, ID recomendación: ' . $ac['id_recom_general'] . '.', 4); ?>
                                        </a>
                                    </td>
                ...
                <div class="row" style="border-radius: 10px 10px 10px 10px;">
                    <div class="col-md-2">
                        <div class="form-group">
                            ...
                            <input type="hidden" class="form-control" name="id_recom_general" value="<?php echo $e_detalle['id_recom_general']?>">
                ...
                <div class="form-group clearfix">
                    ...
                    <button type="submit" name="add_acuerdo_rec_gral" class="btn btn-primary" value="subir">Guardar</button>
                </div>
            </form>
        </div>
    </div>
