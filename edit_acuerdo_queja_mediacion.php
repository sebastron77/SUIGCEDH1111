...
if (isset($_POST['update'])) {
    if (empty($errors)) {
		...
		$name = $_FILES['acuerdo_adjunto_publico']['name'];
        $temp = $_FILES['acuerdo_adjunto_publico']['tmp_name'];
        ...
        $query .= "sintesis_documento='{$sintesis_documento}', ";
        $query .= "acuerdo_adjunto_publico='{$name}' ";
        $query .= "WHERE id_rel_queja_acuerdos='{$db->escape($e_acuerdo['id_rel_queja_acuerdos'])}'";
		 
		$result = $db->query($query);
        if ($result && $db->affected_rows() === 1) {
            //sucess
            $session->msg('s', "Acuerdo de Mediacción de la queja ha actualizado! ");
			insertAccion($user['id_user'],'"'.$user['username'].'" edito el Acuerdo y/o Documento en Mediacción '.$e_acuerdo['tipo_acuerdo'].', del Folio '.$queja['folio_queja'].' (id:'.(int)$e_acuerdo['id_rel_queja_acuerdos'].').',2);
            redirect('acuerdos_queja_mediacion.php?id=' . (int)$e_acuerdo['id_queja_date'], false);
        } else {
            //failed
...
<?php header('Content-Type: text/html; charset=utf-8'); include_once('layouts/header.php'); ?>
        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                <form method="post" action="edit_acuerdo_queja_mediacion.php?id=<?php echo (int)$e_acuerdo['id_rel_queja_acuerdos']; ?>&q=<?php echo (int)$e_acuerdo['id_queja_date']; ?>" class="clearfix" enctype="multipart/form-data">
                    <div class="row">                    
						 <div class="col-md-2">
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="sintesis_documento" id="sintesis_documento" cols="10" rows="3" required><?php echo $e_acuerdo['sintesis_documento']; ?></textarea>
                        </div>
                    </div>
					 ...
					 <div class="form-group clearfix">
                        <a href="acuerdos_queja_mediacion.php?id=<?php echo $id_queja_date?>" class="btn btn-md btn-success" data-toggle="tooltip" title="Regresar">
                            Regresar
                        </a>
