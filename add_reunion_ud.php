...
<?php
$page_title = 'Agregar Reunión de Trabajo de Desaparecidos';
...
if (isset($_POST['add_reunion'])) {	
    ...
    if (empty($errors)) {
        ...
        $year = date("Y");
        // Se crea el folio la Reunon de trabao
        $folio = 'CEDH/' . $no_folio1 . '/' . $year . '-RTUD';

		$dbh1 = new PDO('mysql:host=localhost;dbname=suigcedh', 'suigcedh', '9DvkVuZ915H!');
		
		// Inserto los datos de la reunion		
		$query = "INSERT INTO reuniones_trabajo_ud (";
		$query .= "folio,fecha_reunion, hora_reunion, lugar_reunion, acciones_realizar, observaciones, quien_atendio, no_asistentes, user_creador,fecha_creacion)  VALUES (";
		$query .= " '{$folio}','{$fecha_reunion}','{$hora_reunion}','{$lugar_reunion}','{$acciones_realizar}','{$observaciones}','{$quien_atendio}',{$no_asistentes},{$id_user},NOW()); ";
		...
		if ($dbh1->query($query) && $db->query($query2) ){
			$id_reuniones_trabajo_ud = $dbh1->lastInsertId();
			insertAccion($user['id_user'], '"'.$user['username'].'" agregó una Reunion de Trabajo de la Unidad de Desaparecidos. Folio '.$folio.'.', 1);
		}
		...
		if( $id_reuniones_trabajo_ud > 0 ){
		
		for ($i = 0; $i < sizeof($nombre_participante); $i = $i + 1) {
				if($nombre_participante[$i] != ''){
					$queryInsert = "INSERT INTO rel_reuniones_instituciones_ud (id_reuniones_trabajo_ud,nombre_participante,id_cat_gen,institucion_participante) VALUES('$id_reuniones_trabajo_ud','$nombre_participante[$i]','$id_cat_gen[$i]','$institucion_participante[$i]')";
					if ($db->query($queryInsert)) {
							insertAccion($user['id_user'], '"'.$user['username'].'" agregó una Institución participante en Reunion de Trabajo de la Unidad de Desaparecidos del Folio: '.$folio.'- Institusión '.$institucion_participante[$i].',', 1);
					} else {
						//echo 'falla';
					}
				}		
			}
			$session->msg('s', " La Reunión de Trabajo ha sido agregada con éxito.");
    ...
    <div class="panel panel-default">
        <div class="panel-heading">
            <strong>
                ...
                <span>Agregar Reunión de Trabajo</span>
            </strong>
        </div>
        <div class="panel-body">
            <form method="post" action="add_reunion_ud.php" class="clearfix" >
		...
		<br><br>		
				<div class="form-group clearfix">
                    ...
                    <button type="submit" name="add_reunion" class="btn btn-primary">Guardar</button>
                </div>
            </form>
        </div>
    </div>
