...
$inticadores_pat = find_all_pat_area($area,'rel_accion_atencion');
$tipo_atencion = find_all_order('cat_tipo_atencion', 'id_cat_tipo_atencion');
?>
<?php


if (isset($_POST['add_avance_actividades_visitadurias'])) {
    ...
    if (empty($errors)) {
        ...
        $year = date("Y");
        $folio = 'CEDH/' . $no_folio . '/' . $year . '-AVIST';	
		  ...
		  $dbh = new PDO('mysql:host=localhost;dbname=suigcedh', 'suigcedh', '9DvkVuZ915H!');
				...
				$query .= " '{$folio}','{$mes}','{$ejercicio}',{$area},'{$observaciones}','{$id_user}',NOW() )";
			...
			if ($db->query($query2) ) {
				...
				for ($i = 0; $i < sizeof($id_cat_tipo_atencion); $i = $i + 1) {
					...
					//sucess
					$session->msg('s', " El Registro de Avances Visitadurías ha sido agregado con éxito.");
					insertAccion($user['id_user'], '"' . $user['username'] . '" agregó el Avances Visitadurías, Folio: ' . $folio . '.', 1);
					redirect('avance_actividades_visitadurias.php?a='.$area, false);
				}else{
					$session->msg('d', ' No se pudo agregar el Registro deAvances Visitadurías,debido a que no se genero ID de la misma');
					redirect('add_avance_actividades_visitadurias.php?a='.$area, false);
				}
			}
    ...
    <div class="panel panel-default">
        <div class="panel-heading">
            <strong>
                ...
                <span>Alta de Registro de Avances <?php echo $area_informe['nombre_area'];?></span>
            </strong>
        </div>
        <div class="panel-body">
            <form method="post" action="add_avance_actividades_visitadurias.php?a=<?php echo $area;?>" >
    <div class="row">
            <div class="col-md-6">
...
<?php if ($tipo['visitadurias']==='1') : ?>
                ...
                <div class="form-group clearfix">
                    <a href="avance_actividades_visitadurias.php?a=<?php echo $area;?>" class="btn btn-md btn-success" data-toggle="tooltip" title="Regresar">
                        Regresar
                    </a>
                    <button type="submit" name="add_avance_actividades_visitadurias" class="btn btn-primary" value="subir">Guardar</button>
                </div>
            </form>
        </div>
    </div>
