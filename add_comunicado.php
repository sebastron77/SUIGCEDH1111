...
if (isset($_POST['add_comunicado'])) {
    ...
    if (empty($errors)) {
        ...
        $folio = 'CEDH/' . $no_folio1 . '/' . $year . '-COM';
            ...
            $query .= "folio,fecha_publicacion,tipo_nota,nombre_nota,url_acceso,observaciones,id_indicadores_pat,user_creador,fecha_creacion";
            $query .= ") VALUES (";
            $query .= " '{$folio}','{$fecha_publicacion}','{$tipo_nota}','{$nombre_nota}','{$url_acceso}','{$observaciones}','{$id_indicadores_pat}','{$id_user}',NOW()); ";
