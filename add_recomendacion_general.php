    ...
    if (empty($errors)) {
	   ...
	   /*creo archivo index para que no se muestre el Index Of*/
        ...
        $dbh->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            ...
            $query .= "numero_recomendacion,autoridad_responsable,fecha_recomendacion,observaciones,recomendacion_adjunto,recomendacion_adjunto_publico,sintesis_rec,traduccion,lectura_facil,hecho_completo,infografia,user_creador,fecha_creacion";
            $query .= ") VALUES (";
            $query .= " '{$numero_recomendacion}','{$autoridad_responsable}','{$fecha_recomendacion}','{$observaciones}','{$name}','{$name2}','{$nameRecSint}','{$nameRecTrad}','{$nameRecLF}','{$hecho_completo}','{$infografia}',{$id_user},NOW() ";
            $query .= ")";
	   ...
	   $dbh->exec($query);
		...
		for ($i = 0; $i < sizeof($derecho_vuln); $i++) {						
				$query2 = "INSERT INTO rel_recomendacion_gral_der_vuln(id_recom_general, id_cat_der_vuln) VALUES ($id_insertado,$derecho_vuln[$i]); ";											
				$db->query($query2);
			}		
	   
			$session->msg('s', " La recomendación General ha sido agregada con éxito.");
            insertAccion($user['id_user'], '"' . $user['username'] . '" agregó recomendación general, Núm. Rec.: ' . $numero_recomendacion . '.', 1);
                   ...
                   <div class="col-md-3">
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="autoridad_responsable" placeholder="Autoridad(es) Responsable(es)" required>
                        </div>
                    </div>                   
                </div>
