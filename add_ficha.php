...
if (isset($_POST['add_ficha'])) {
    ...
    if (empty($errors)) {
        ...
        if ($move && $name != '') {
            ...
            $query .= "folio,id_cat_funcion,id_paciente,id_area_solicitante,fecha_intervencion,resultado,documento_emitido,ficha_adjunto,fecha_creacion,tipo_ficha,nombre_especialista,clave_documento,id_indicadores_pat,user_creador";
            $query .= ") VALUES (";
            $query .= " '{$folio}','{$funcion}','{$id_paciente}','{$area_solicitante}','{$fecha_intervencion}','{$resultado}','{$documento_emitido}','{$name}',NOW(),1,'{$nombre_especialista}','{$clave_documento}','{$id_indicadores_pat}','{$id_user}'";
            $query .= ")";
            ...
            $query = "INSERT INTO fichas (";
            $query .= "folio,id_cat_funcion,id_paciente,id_area_solicitante,fecha_intervencion,resultado,documento_emitido,fecha_creacion,tipo_ficha,nombre_especialista,clave_documento,id_indicadores_pat,user_creador";
            $query .= ") VALUES (";
            $query .= " '{$folio}','{$funcion}','{$id_paciente}','{$area_solicitante}','{$fecha_intervencion}','{$resultado}','{$documento_emitido}',NOW(),1,'{$nombre_especialista}','{$clave_documento}','{$id_indicadores_pat}','{$id_user}'";
            $query .= ")";
        ...
        if ($db->query($query) && $db->query($query2)) {
            ...
            insertAccion($user['id_user'], '"' . $user['username'] . '" agregó Ficha Médica, Folio: ' . $folio . '.', 1);
		  ...
		  $("#id_paciente").change(function() {
            $("#id_paciente option:selected").each(function() {
                id_paciente = $(this).val();
					window.location.href = 'add_ficha.php?num_queja=' + $(this).val();
            });
        })
});
		/*function buscar() {
        var opcion = document.getElementById('num_queja').value;
        window.location.href = 'add_ficha.php?num_queja=' + opcion;
    }*/
</script>
                ...
                <div class="row">
                    <div class="col-md-2">
                        <div class="form-group">
                            ...
                            <select class="form-control" name="num_queja" id="num_queja"  required>
