<?php
$page_title = 'Agregar Institución de Procedencia';
...
if (isset($_POST['add'])) {
    ...
    if (empty($errors)) {
        ...
        $query  = "INSERT INTO cat_instituciones (";
        ...
        if ($db->query($query)) {
            //sucess
            $session->msg('s', "Institución de Procedencia creada! ");
			insertAccion($user['id_user'],'"'.$user['username'].'" creo la Institución de Procedencia -'.$name.'-.',1);
            redirect('add_instituciones.php', false);
        } else {
            //failed
            $session->msg('d', 'Lamentablemente no se pudo dar de alta la Institución de Procedencia!');
            redirect('add_instituciones.php', false);
        }
...
<div class="login-page">
    <div class="text-center">
        <h3>Agregar Institución de Procedencia</h3>
    </div>
    <?php echo display_msg($msg); ?>
    <form method="post" action="add_instituciones.php" class="clearfix">
