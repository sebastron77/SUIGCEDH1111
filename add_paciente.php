...
if (isset($_POST['add_paciente'])) {
    if (empty($errors)) {
        ...
        //$folio_expediente = remove_junk($db->escape($_POST['folio_expediente']==0 ? NULL : $_POST['folio_expediente']  ));

        $query = "INSERT INTO paciente (";
        $query .= "nombre, paterno, materno, genero, edad, nacionalidad, municipio, entidad, escolaridad, ocupacion, discapacidad,  grupo_vulnerable, leer_escribir, comunidad, telefono, email,  tipo_expediente";
		if($folio_expediente >0){
			$query .= " ,folio_expediente";
		}
        $query .= ") VALUES (";
        $query .= "'{$nombre}', '{$paterno}', '{$materno}', '{$genero}', '{$edad}', '{$nacionalidad}', '{$municipio1}', '{$entidad1}', '{$escolaridad}', '{$ocupacion}', '{$discapacidad}', '{$grupo_vulnerable}', '{$leer_escribir}', '{$comunidad}', '{$telefono}', '{$email}', '{$tipo_expediente}'";
		if($folio_expediente >0){
			$query .= " ,".$folio_expediente;
		}
        $query .= ")";
...
<script>
    $(document).ready(function() {
		...
		$("#ejercicio").change(function() {
            $("#ejercicio option:selected").each(function() {
                tipo_expediente = $("#tipo_expediente").val();
                ejercicio = $(this).val();
                $.post("get_exp.php", {
                    tipo_expediente: tipo_expediente,
                    ejercicio: ejercicio
				 ...
				 <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <select class="form-control" id="tipo_expediente" name="tipo_expediente">
                                ...
                                <option value="N">Sin Expediente</option>
                            </select>
                        </div>
                    </div>
					<div class="col-md-4">
