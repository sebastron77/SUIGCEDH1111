...
$all_reclasificacion =count_by_reclasificacion( $e_detalle['id_queja_date']);
$all_clasificacion =count_by_clasificacion( $e_detalle['id_queja_date']);
...
if (isset($_POST['procesal_queja'])) {
    ...
    if (empty($errors)) {
		...
		$reclasificacion = $_POST['reclasificacion']? 1 : 0;
		//$reclasificacion = $_POST['reclasificacion'] == 'on' ? 1 : 0;
		//$reclasificacion = $_POST['reclasificacion'];
       ...
       if($reclasificacion === 0){
		...
		$session->msg('s', " Los datos de los Derechos Presuntamente Violentados se han sido agregado con éxito.");
	   ...
	   }else{//si es reclasificación
		   
		   ///obtengo los derechos con los que cuenta actualmente la queja
		   $derecho_vulnerado_old = find_by_violentados_all('rel_queja_der_vuln', 'cat_der_vuln', $e_detalle['id_queja_date']);
		   $derecho_general_old = find_by_violentados_all('rel_queja_der_gral', 'cat_derecho_general', $e_detalle['id_queja_date']);
		   		   
		//inserto los derechos con los que cuenta actualmente la queja al historico
		   foreach ($derecho_vulnerado_old as $derecho) :
		   $id_rel_queja_reclasificacion=$derecho['id_rel_queja_reclasificacion']==''?"NULL":$derecho['id_rel_queja_reclasificacion'];
				$query = "INSERT INTO rel_queja_der_vuln_history (id_queja_date, id_cat_der_vuln, id_rel_queja_reclasificacion) VALUES($id,". $derecho['id_campo'].",".$id_rel_queja_reclasificacion .");";			        	
				if ($db->query($query)) {
					//echo "<p>Denuncia-denunciado insertada con éxito</p>";		
				 }
			endforeach;
		
		 foreach ($derecho_general_old as $derecho):
		   $id_rel_queja_reclasificacion=$derecho['id_rel_queja_reclasificacion']==''?"NULL":$derecho['id_rel_queja_reclasificacion'];
				$query = "INSERT INTO rel_queja_der_gral_history (id_queja_date, id_cat_derecho_general, id_rel_queja_reclasificacion) VALUES($id,". $derecho['id_campo'].",". $id_rel_queja_reclasificacion.");";			        
				if ($db->query($query)) {
					//echo "<p>Denuncia-denunciado insertada con éxito</p>";		
				 }
		endforeach;
		
		//elimino los derechos con los que cuenta actualmente la queja de las tablas principales
		   ...
		   $dbh->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			...
			//inserto los datos de la rreclasificación
		   $query = "INSERT INTO rel_queja_reclasificacion (id_queja_date,fecha_reclasificacion,observaciones_reclasificacion) VALUES ('{$id}',NOW(),'{$observaciones_reclasificacion}')";
		...
		for ($i = 0; $i < sizeof($id_cat_der_vuln); $i = $i + 1) {
			...
			$session->msg('s', " Los datos de la reclasificación se han sido agregado con éxito.");
			insertAccion($user['id_user'], '\"' . $user['username'] . '\" reclasifico los Derechos Presuntamente Violentados del expediene ' . $folio_editar . '.', 2);
			redirect('procesal_queja.php?id=' . (int) $e_detalle['id_queja_date'], false);
		   }
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            <form method="post" action="procesal_queja.php?id=<?php echo (int) $e_detalle['id_queja_date']; ?>" enctype="multipart/form-data">
				...
				<?php 
					if((int) $all_clasificacion['total'] == 0){ ///si la queja NO tiene derechos ya asignados	
				?>
					 <div class="row">
					 <input id="reclasificacion" name="reclasificacion" type="hidden" value="0" />
				...
				<?php }else{ 
						...
						if((int) $all_reclasificacion['total'] == 0){
					?>	
					<div class="row">
					 <div class="panel-heading">
						<strong>
					...
					<table style="color:#3a3d44; margin-top: -10px; page-break-after:always;" >
						<tr>
							<td style="width: 50%;"><br><label for="derecho_general">Derecho general </label> </td>
							<td style="width: 50%;"><br><label for="derecho_violentado">Derecho violentado </label> </td>
						</tr>
						<tr>
							<td style="width: 50%;">
							<?php foreach ($derecho_general as $general) : ?>
								<div class="col-md-7">
									<div class="form-group">									
							...
							<td style="width: 50%;">
							<?php foreach ($derecho_vulnerado as $general) : ?>
							<div class="col-md-7">
									<div class="form-group">
					...
					<?php
							...
							$derecho_vulnerado_inicial = find_by_violentados('rel_queja_der_vuln_history', 'cat_der_vuln', $e_detalle['id_queja_date'],0);
							$derecho_general_inicial = find_by_violentados('rel_queja_der_gral_history', 'cat_derecho_general', $e_detalle['id_queja_date'],0);
							$reclasificacion_queja= find_reclasificacion_queja($e_detalle['id_queja_date']);
							...
							<div class="row">
								...
								<table style="color:#3a3d44; margin-top: -10px; page-break-after:always;" >
									...
									<tr>
										<td style="width: 50%;">
										<?php foreach ($derecho_general_inicial as $general) : ?>
											<div class="col-md-7">
												<div class="form-group">									
													<li><?php foreach ($cat_derecho_gral as $datos) {
																if ($datos['id_cat_derecho_general'] === $general['id_cat_derecho_general'])
										...
										<td style="width: 50%;">
										<?php foreach ($derecho_vulnerado_inicial as $general) : ?>
										<div class="col-md-7">
												<div class="form-group">
													<li><?php foreach ($cat_derecho_vuln as $datos) {
																if ($datos['id_cat_der_vuln'] === $general['id_cat_der_vuln'])
						 ...
						 <br><br>
							 <div class="panel-heading">
								<strong>
									<span class="glyphicon glyphicon-th"></span>
									<span> Reclasificación</span>
								</strong>
							</div>
							
							<?php foreach ($reclasificacion_queja as $datos):
									$derecho_vulnerado = find_by_violentados('rel_queja_der_vuln_history', 'cat_der_vuln', $e_detalle['id_queja_date'],$datos['id_rel_queja_reclasificacion']);
									$derecho_general = find_by_violentados('rel_queja_der_gral_history', 'cat_derecho_general', $e_detalle['id_queja_date'],$datos['id_rel_queja_reclasificacion']);
				 
											
				?>
									<table style="color:#3a3d44; margin-top: -10px; page-break-after:always;" >
										<tr>
											<td style="width: 50%;"><br><label for="fecha"><strong><span class="glyphicon glyphicon-play"></span></strong>&nbsp;Fecha de Reclasificación:&nbsp; <?php echo $datos['fecha_reclasificacion']; ?></label> </td>
											<td style="width: 50%;"><br><label for="observaciones">Observaciones: &nbsp;<?php echo $datos['observaciones_reclasificacion']; ?> </label> </td>
										</tr>
										<tr>
											<td style="width: 50%;"><br><label for="derecho_general">Derecho general </label> </td>
											<td style="width: 50%;"><br><label for="derecho_violentado">Derecho violentado </label> </td>
					...
					<?php  endforeach;
						...
						$derecho_vulnerado_Act = find_by_violentadosAct('rel_queja_der_vuln', 'cat_der_vuln', $e_detalle['id_queja_date']);
						$derecho_general_Act = find_by_violentadosAct('rel_queja_der_gral', 'cat_derecho_general', $e_detalle['id_queja_date']);
						?>
									<table style="color:#3a3d44; margin-top: -10px; page-break-after:always;" >
										...
										<tr>
											<td style="width: 50">
												<?php foreach ($derecho_general_Act as $general) : ?>
												<div class="col-md-7">
													<div class="form-group">									
														<li><?php foreach ($cat_derecho_gral as $datos) {
																	if ($datos['id_cat_derecho_general'] === $general['id_cat_derecho_general'])
											...
											<td style="width: 50%;">
												<?php foreach ($derecho_vulnerado_Act as $general) : ?>
												<div class="col-md-7">
														<div class="form-group">
															<li><?php foreach ($cat_derecho_vuln as $datos) {
																		if ($datos['id_cat_der_vuln'] === $general['id_cat_der_vuln'])
				...
				<br><br>
						<div class="row">
							<div class="col-md-12">
								<div class="form-group">
									<label for="reclasificacion">¿Es una Reclasificación?</label><br>
									<input id="reclasificacion" name="reclasificacion" type="hidden" value="1" />
									<label class="switch" style="float:left;">
										<div class="row">
											<input type="checkbox" id="reclasificacion1" name="reclasificacion1" checked disabled>
											<span class="slider round"></span>
											<div>
												<p style="margin-left: 150%; margin-top: -3%; font-size: 14px;">Sí</p>
											</div>
										</div>
									</label>
								</div>
						...
						<br>
						<div class="row" id="reclasi" style="display:block">
				...
				<div class="form-group clearfix">
                    <a href="quejas.php?anio=<?php echo $anio?>" class="btn btn-md btn-success" data-toggle="tooltip" title="Regresar">
                        Regresar
                    </a>
