    ...
    if (empty($errors)) {
        ...
        $articulo_causal   = remove_junk($db->escape($_POST['articulo_causal']));
			...
			$result = $db->query($sql);
				...
				insertAccion($user['id_user'], '"' . $user['username'] . '" edito el Recuirso('.$id.') con Folio: -' . $recurso['folio_accion'], 2);
				$session->msg('s', " El Recurso con folio '" . $recurso['folio_accion'] . "' ha sido acuatizado con éxito.");
