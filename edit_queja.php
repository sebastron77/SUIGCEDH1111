    ...
    if (empty($errors)) {
        ...
        if ($name != '') {
            $sql = "UPDATE quejas_dates SET fecha_presentacion='{$fecha_presentacion}', id_cat_med_pres='{$id_cat_med_pres}', id_cat_aut='{$id_cat_aut}', archivo='{$name}',id_tipo_ambito='{$id_tipo_ambito}', 
                    observaciones='{$observaciones}', id_cat_quejoso='$id_cat_quejoso', id_cat_agraviado='$id_cat_agraviado', id_user_asignado='$id_user_asignado', localidad='{$localidad}',
        ...
        if ($name == '') {
            $sql = "UPDATE quejas_dates SET fecha_presentacion='{$fecha_presentacion}', id_cat_med_pres='{$id_cat_med_pres}', id_cat_aut='{$id_cat_aut}',id_tipo_ambito='{$id_tipo_ambito}', 
                    observaciones='{$observaciones}', id_cat_quejoso='$id_cat_quejoso', id_cat_agraviado='$id_cat_agraviado', id_user_asignado='$id_user_asignado', localidad='{$localidad}',
                    id_area_asignada='$id_area_asignada', id_estatus_queja=NULL, estado_procesal='{$id_cat_est_procesal}', dom_calle='$dom_calle', dom_numero='$dom_numero', dom_colonia='$dom_colonia', 
...
//echo $sql;
        ...
        if (($result && $db->affected_rows() === 1) && ($result2 && $db->affected_rows() === 1)) {
            ...
            redirect('quejas.php?anio='.$anio, false);
        } else {
        ...
        <div class="panel-body">
            <form method="post" action="edit_queja.php?id=<?php echo (int) $e_detalle['id_queja_date']; ?>" enctype="multipart/form-data">
                <h3 style="font-weight:bold;">
                    ...
                    Generales de la Queja <?php echo $anio?>
                </h3>
                <div class="row">
                    <div class="col-md-3">
                ...
                <div class="row">
                    ...
                    <div class="col-md-2">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="localidad" placeholder="Localidad" value="<?php echo $e_detalle['localidad'] ?>">
                        </div>
                    </div>
                </div>
                <div class="row">
