...
if (isset($_POST['add_entrevista'])) {
    ...
    if (empty($errors)) {
        ...
        $folio = 'CEDH/' . $no_folio1 . '/' . $year . '-ENTR';
            ...
            $query .= "folio,fecha_entrevista,tema_entrevista,lugar_entrevista,nombre_entrevistado,cargo_entrevistado,temas_destacados,observaciones,id_indicadores_patuser_creador,fecha_creacion";
            $query .= ") VALUES (";
            $query .= " '{$folio}','{$fecha_entrevista}','{$tema_entrevista}','{$lugar_entrevista}','{$nombre_entrevistado}','{$cargo_entrevistado}','{$temas_destacados}','{$observaciones}','{$id_indicadores_pat}','{$id_user}',NOW()); ";
