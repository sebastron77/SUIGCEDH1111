		...
		$(document).on('click', '#removeRow2', function() {
			...
			$("#reclasificacion").change(function(){
			  if($("#reclasificacion"). prop("checked") == true){
				/alert("Activado");
document.getElementById('reclasi').style.display = 'block';
document.querySelector('#observaciones_reclasificacion').required = true;
document.querySelector('#id_cat_derecho_general').required = true;
document.querySelector('#id_cat_derecho_vuln').required = true;
			  }else{
document.getElementById('reclasi').style.display = 'none';
document.querySelector('#observaciones_reclasificacion').required = false;
document.querySelector('#id_cat_derecho_general').required = false;
document.querySelector('#id_cat_derecho_vuln').required = false;
}
...
<?php if((int) $all_clasificacion['total'] > 0){ ///la queja tiene derechos ya asignados?
	$existe=1;
?>				
...
<?php } ?>				
<?php if((int) $all_reclasificacion['total'] > 0){ 
...
?>				
				 <div class="row">
					...
					$derecho_vulnerado = find_by_violentados('rel_queja_der_vuln', 'cat_der_vuln', $e_detalle['id_queja_date'],$datos['id_rel_queja_reclasificacion']);
					$derecho_general = find_by_violentados('rel_queja_der_gral', 'cat_derecho_general', $e_detalle['id_queja_date'],$datos['id_rel_queja_reclasificacion']);
 
							
?>
					<table style="color:#3a3d44; margin-top: -10px; page-break-after:always;" >
						<tr>
							<td style="width: 50%;"><br><label for="fecha">Fecha de Reclasificación: <?php echo $datos['fecha_reclasificacion']; ?></label> </td>
							...
							<div class="col-md-12">
								<div class="form-group">
									...
									<label class="switch" style="float:left;">
										<div class="row">
											<input type="checkbox" id="reclasificacion" name="reclasificacion" checked>
