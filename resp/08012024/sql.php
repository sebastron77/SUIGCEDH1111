...
{
  $sql = "SELECT a.*,b.`nombre_area`,c.nombre_area as nombre_area_creacion FROM correspondencia a LEFT JOIN `area` b ON a.`id_area_turnada`=b.`id_area`  LEFT JOIN `area` c ON a.`area_creacion`=c.`id_area` WHERE id_area_turnada='{$area}' ORDER BY fecha_recibido DESC";
  $result = find_by_sql($sql);
  return $result;
}
...
function find_all_env_correspondencia2($area)
{
  ...
  b.nombre_area as nombre_area_creacion,IF(accion_realizada is null,'No','Si') as accion_realizada ,IF(accion_realizada is null,'0','1') as atendida
		  ...
		  WHERE se_turna_a_area='{$area}' ORDER BY fecha_en_que_se_turna DESC";
  $result = find_by_sql($sql);
  return $result;
}
...
/* Funcion que encuentra todas las correspondencias */
/*--------------------------------------------------*/
function find_all_env_correspondenciaExter($area)
{
  $sql = "SELECT 
