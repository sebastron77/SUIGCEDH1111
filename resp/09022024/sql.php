...
function find_all_env_correspondencia($area,$year)
{
  ...
  IFNULL(c.fecha_espera_respuesta,'') as fecha_espera_respuesta, c.asunto, c.medio_envio, a.nombre_area ,IF(accion_realizada is null,'No','Si') as accion_realizada
          FROM envio_correspondencia c
          LEFT JOIN area a ON c.se_turna_a_area = a.id_area WHERE area_creacion='{$area}' AND YEAR(fecha_emision)='".$year."'";
  $result = find_by_sql($sql);
  return $result;
}
/*----------------------------------------------------------------------------------*/
...
FROM `rel_indicadores_calendarizacion`
WHERE tipo='".$tipo."'
  AND id_indicadores_pat='{$db->escape($id)}' AND vigente=1 LIMIT 1");
  if ($result = $db->fetch_assoc($sql))
    return $result;
  else
    return null;
...
/*--------------------------------------------------------------*/
function find_all_indicadores_real($table,$campo,$fecha,$id_indicador)
{
  global $db;
    return find_by_sql(" SELECT 
