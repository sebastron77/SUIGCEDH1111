...
if (isset($_POST['edit_actuacion'])) {
    ...
    if (empty($errors)) {
        ...
        if ($name != '') {
            $sql = "UPDATE actuaciones SET fecha_captura_acta='{$fecha_captura_acta}', catalogo='{$catalogo}', descripcion='{$descripcion}', autoridades='{$autoridades}',nombre_desaparecido='{$nombre_desaparecido}', autoridades_federales='{$autoridades_federales}', num_exp_origen='{$num_exp_origen}', peticion='{$peticion}', adjunto='{$name}' WHERE id_actuacion='{$db->escape($id)}'";
        }
        if ($name == '') {
            $sql = "UPDATE actuaciones SET fecha_captura_acta='{$fecha_captura_acta}', catalogo='{$catalogo}', descripcion='{$descripcion}', autoridades='{$autoridades}', nombre_desaparecido='{$nombre_desaparecido}',autoridades_federales='{$autoridades_federales}', num_exp_origen='{$num_exp_origen}', peticion='{$peticion}' WHERE id_actuacion='{$db->escape($id)}'";
        }
        $result = $db->query($sql);
