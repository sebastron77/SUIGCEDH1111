...
function find_all_quejas($id, $user,$anio)
{
  ...
  ep.descripcion as est_proc, ctr.descripcion as id_tipo_resolucion,ar.nombre_area, CONCAT(du.nombre,' ',du.apellidos) as user_asignado ";
...
function find_all_quejas_lc($anio)
{
  ...
  $sql .= " cq.paterno as paterno_quejoso, cq.materno as materno_quejoso, CONCAT(ca.`nombre`,' ',ca.`paterno`,' ',ca.`materno`) as nombre_agraviado,q.fecha_creacion, q.archivo, ep.descripcion as est_proc, ctr.descripcion as id_tipo_resolucion,ar.nombre_area, CONCAT(du.nombre,' ',du.apellidos) as user_asignado ";
  $sql .= " FROM quejas_dates q";
  $sql .= " LEFT JOIN cat_medio_pres mp ON mp.id_cat_med_pres = q.id_cat_med_pres";
  $sql .= " LEFT JOIN cat_autoridades au ON au.id_cat_aut = q.id_cat_aut";
  $sql .= " LEFT JOIN cat_quejosos cq ON cq.id_cat_quejoso = q.id_cat_quejoso";
