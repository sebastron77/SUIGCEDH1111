...
function find_all_quejas_lc($anio)
{
  ...
  q.archivo, ep.descripcion as est_proc, ctr.descripcion as id_tipo_resolucion,ar.nombre_area, CONCAT(du.nombre,' ',du.apellidos) as user_asignado,fecha_actualizacion ";
  $sql .= " FROM quejas_dates q";
  $sql .= " LEFT JOIN cat_medio_pres mp ON mp.id_cat_med_pres = q.id_cat_med_pres";
  $sql .= " LEFT JOIN cat_autoridades au ON au.id_cat_aut = q.id_cat_aut";
  $sql .= " LEFT JOIN cat_quejosos cq ON cq.id_cat_quejoso = q.id_cat_quejoso";
...
function find_all_consejo( $year)
{
  global $db;
  $results = array();
  $sql = "SELECT * FROM consejo WHERE folio LIKE '%/{$year}-%'";
  $result = find_by_sql($sql);
  return $result;
}
/*----------------------------------------------------------------------------------*/
...
function find_all_capacitaciones($ejercicio)
{
  global $db;
  $results = array();
  $sql = "SELECT * FROM capacitaciones WHERE folio LIKE '%/{$ejercicio}-%' ORDER BY fecha";
  $result = find_by_sql($sql);
  return $result;
}
...
function find_all_capacitaciones_area($area,$ejercicio)
{
  global $db;
  $results = array();
  $sql = "SELECT * FROM capacitaciones WHERE area_creacion = '{$area}' AND folio LIKE '%/{$ejercicio}-%' ORDER BY fecha";
  $result = find_by_sql($sql);
  return $result;
}
...
{
  $sql = "SELECT a.*, b.nombre_area FROM eventos a  LEFT JOIN area as b ON a.area_creacion = b.id_area WHERE folio LIKE '%/".$year."-%' ";
  $result = find_by_sql($sql);
  return $result;
}
...
{
  $sql = "SELECT c.id_env_corresp, IFNULL(no_oficio,'S/N') as no_oficio,c.folio, IFNULL(c.fecha_en_que_se_turna,'') as fecha_en_que_se_turna, 
...
function find_all_env_correspondencia($area,$year)
{
  ...
  IFNULL(c.fecha_espera_respuesta,'') as fecha_espera_respuesta, c.asunto, c.medio_envio, a.nombre_area ,IF(accion_realizada is null,'No','Si') as accion_realizada
          FROM envio_correspondencia c
          LEFT JOIN area a ON c.se_turna_a_area = a.id_area WHERE area_creacion='{$area}' AND c.folio LIKE '%/".$year."-%'";
  $result = find_by_sql($sql);
  return $result;
}
/*----------------------------------------------------------------------------------*/
...
{
  $sql = "SELECT a.*, b.nombre_area FROM eventos a  LEFT JOIN area as b ON a.area_creacion = b.id_area WHERE area_creacion=".$id_area." AND folio LIKE '%/".$year."-%'";
  $result = find_by_sql($sql);
  return $result;
}
...
{
  $sql = "SELECT * FROM jornadas WHERE folio LIKE '%/".$year."-%'";
  $result = find_by_sql($sql);
  return $result;
}
  /*----------------------------------------------------------------------------*/
...
/* Funcion que encuentra todas las correspondencias */
/*--------------------------------------------------*/
function find_all_elemcursos($id,$tabla,$relacion,$campo)
{
  $sql = "SELECT
			a.id_".$relacion.",
			".$campo.", 
