...
function find_all_actuaciones($anio)
{
  ...
  WHERE  folio_actuacion LIKE '%/{$anio}-%' ";
  $result = find_by_sql($sql);
  return $result;
}
...
{
  $sql = "SELECT DISTINCT folio_expediente,f.folio  FROM paciente pa LEFT JOIN folios f ON f.id_folio = pa.folio_expediente ; ";
  $result = find_by_sql($sql);
  return $result;
}
...
function find_all_Colaboraciones()
{
  ...
  IFNULL(CONCAT(c.nombre,' ',c.paterno,' ',c.materno),'') as victima_atendida, 
