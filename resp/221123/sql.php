...
function find_by_violentados($tableA, $tableB, $id)
{
  global $db;
  $id = (int)$id;
  $sql = "SELECT id_{$db->escape($tableB)} ,IFNULL(descripcion,'')as descripcion FROM {$db->escape($tableA)} LEFT JOIN {$db->escape($tableB)} USING(id_{$db->escape($tableB)}) WHERE id_queja_date='{$db->escape($id)}'";
	$result = find_by_sql($sql);
  return $result;
}
...
function find_all_quejas($id, $user,$anio)
{
  ...
  $sql .= " AND q.folio_queja LIKE '%/{$anio}-%'  ORDER BY q.folio_queja DESC";
  $result = find_by_sql($sql);
  return $result;
}
...
function find_all_orientaciones($anio)
{
  ...
  $sql .= " LEFT JOIN detalles_usuario as d ON d.id_det_usuario = u.id_detalle_user WHERE tipo_solicitud=1 AND o.folio LIKE '%/{$anio}-%' ORDER BY o.folio DESc ;";
  return find_by_sql($sql);
}
...
function find_all_canalizaciones($anio)
{
  ...
  // $sql .= " LEFT JOIN cat_municipios as mp ON mp.id_cat_mun = o.id_cat_mun ";
  $sql .= " LEFT JOIN detalles_usuario as d ON d.id_det_usuario = u.id_detalle_user WHERE tipo_solicitud=2 AND o.folio LIKE '%/{$anio}-%' ORDER BY o.folio DESc  ;";
  return find_by_sql($sql);
}
...
{
  global $db;
  $sql  = "SELECT id_rel_queja_acuerdos,tipo_acuerdo,fecha_acuerdo,acuerdo_adjunto,acuerdo_adjunto_publico,sintesis_documento,publico,origen_acuerdo,IFNULL(documento_promocion,'') as documento_promocion FROM `rel_queja_acuerdos` WHERE id_queja_date= {$id} ORDER By fecha_acuerdo ASC";
  return $db->query($sql);
}
...
{
  $sql = "SELECT * FROM convenios ORDER BY folio_solicitud DESC";
  $result = find_by_sql($sql);
  return $result;
}
...
{
  $sql = "SELECT a.*,b.`nombre_area`, c.nombre_area as nombre_area_creacion FROM correspondencia a LEFT JOIN `area` b ON a.`id_area_turnada`=b.`id_area` LEFT JOIN `area` c ON a.`area_creacion`=c.`id_area`";
  $result = find_by_sql($sql);
  return $result;
}
/*-------------------------------------------------------------*/
...
{
  $sql = "SELECT a.*,b.`nombre_area`,c.nombre_area as nombre_area_creacion FROM correspondencia a LEFT JOIN `area` b ON a.`id_area_turnada`=b.`id_area`  LEFT JOIN `area` c ON a.`area_creacion`=c.`id_area` WHERE id_area_turnada='{$area}'";
  $result = find_by_sql($sql);
  return $result;
}
...
{
  $sql = "SELECT c.id_env_corresp, IFNULL(no_oficio,'S/N') as no_oficio,c.folio, IFNULL(c.fecha_en_que_se_turna,'') as fecha_en_que_se_turna, IFNULL(c.fecha_espera_respuesta,'') as fecha_espera_respuesta, c.asunto, c.medio_envio, a.nombre_area 
...
function find_by_id_env_correspondencia($id)
{
  ...
  $sql = $db->query("SELECT c.id_env_corresp, c.folio, c.fecha_emision, c.tipo_tramite, c.oficio_enviado, c.fecha_en_que_se_turna, c.fecha_espera_respuesta, 
                            ...
                            c.area_creacion, c.fecha_creacion, c.user_creador, c.se_turna_a_area, d.nombre, d.apellidos, atu.nombre_area as nomat,no_oficio
                    FROM envio_correspondencia c
                    LEFT JOIN area a ON c.se_turna_a_area = a.id_area 
...
b.nombre_area as nombre_area_creacion  
          FROM envio_correspondencia c
          LEFT JOIN area a ON c.se_turna_a_area = a.id_area 
...
WHERE tipo_ficha = 1 AND  f.folio LIKE '%/{$anio}-%' ;";
  $result = find_by_sql($sql);
  return $result;
}
/*----------------------------------------------------------------*/
...
LEFT JOIN paciente p ON p.id_paciente = f.id_paciente
LEFT JOIN folios ff ON ff.id_folio = p.folio_expediente
WHERE tipo_ficha = 2 AND user_creador = '{$id_user}' AND f.folio LIKE '%/{$anio}-%' ;";
  $result = find_by_sql($sql);
  return $result;
}
...
/*---------------------------------------------------------------------------------*/
function find_recomendacion_der_vuln($id)
{
  global $db;
...
function find_all_actividadesUD()
{
  ...
  CONCAT(c.nombre,' ',c.paterno,' ',c.materno) as victima_atendida, 
...
function find_ficha_completa($id_ficha)
{
  global $db;
       ...
       f.nombre_especialista,f.clave_documento,f.id_cat_funcion,f.id_area_solicitante,
...
/*----------------------------------------------*/
function find_all_Colaboraciones()
{
  global $db;
  $results = array();
  $sql = "SELECT
	id_colaboraciones,folio,
