...
/*creo archivo index para que no se muestre el Index Of*/
        ...
        if ($db->query($query) && $db->query($query2)) {
			...
			if($envio_area === 1){
				...
				$query3 .= "folio,no_oficio,fecha_emision,asunto,medio_envio,se_turna_a_area,tipo_tramite,observaciones,area_creacion,fecha_creacion,user_creador,fecha_en_que_se_turna,fecha_espera_respuesta,envio_oficialia";
				if ($name_enviado != ''){
					$query3 .= ", oficio_enviado ";
				}	
				$query3 .= ") VALUES (";
				$query3 .= " '{$folio}','{$oficio_interno}','{$fecha_en_que_se_turna}','{$asunto}','{$medio_entrega}','{$id_area_turnada}','{$tipo_tramite}','{$observaciones}','{$area_ingreso}',NOW(),'{$detalle}','{$fecha_en_que_se_turna}','{$fecha_espera_respuesta}',1";
				if ($name_enviado != ''){
					$query3 .= ", '{$name_enviado}' ";
				}
				$query3 .= ")";
